...
We will see a quick setup to configure the ``tool_pipx`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_pipx`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_pipx`` formulas under ``/srv/salt/tool_pipx/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_pipx/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_pipx/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_pipx/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_pipx/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_pipx`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_pipx`` formulas under ``/srv/salt/tool_pipx/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_pipx/parameters/roles
...
We create a configuration for the role ``tool_pipx/server`` in ``/srv/salt/tool_pipx/parameters/roles/tool_pipx/server.yaml``:
...
We create another configuration for the role ``tool_pipx/client`` in ``/srv/salt/tool_pipx/parameters/roles/tool_pipx/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_pipx/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_pipx/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_pipx/parameters/dns:domain/  '/srv/salt/tool_pipx/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-pipx-as-json:
      test.nop:
        - name: {{ pipx | json }}
...
.. code-block:: sls
    ...
    pipx-config-file-file-managed:
      file.managed:
        - name: {{ pipx.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='pipx-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ pipx.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``pipx`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ pipx['winner'] }}
